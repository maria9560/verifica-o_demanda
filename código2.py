...
def resolver_coluna(df: pd.DataFrame, col_canonica: str) -> str | None:
    """Retorna o nome real da coluna no df, testando aliases."""
...
with st.sidebar:
    ...
    st.markdown("🟠 **Oper:** `Numero` · `Data/Hora Limite` ou `Data Limite`")
    st.markdown("---")
...
if arquivo1 and arquivo2:
    try:
        ...
        # Resolve aliases antes de validar
        col_srv1 = resolver_coluna(df1, col_srv1) or col_srv1
        col_dt1  = resolver_coluna(df1, col_dt1)  or col_dt1
        col_srv2 = resolver_coluna(df2, col_srv2) or col_srv2
        col_dt2  = resolver_coluna(df2, col_dt2)  or col_dt2
