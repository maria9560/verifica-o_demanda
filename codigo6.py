...
def gauge(titulo: str, valor: float, cor: str) -> go.Figure:
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            "steps": [
                ...
                {"range": [30, 60], "color": {"color": "#fff3cd"}},
    ...
    fig.update_layout(margin=dict(t=80, b=30, l=30, r=30), height=260)
    return fig
...
def exibir_dashboard(
    data_filtro,
    total_cbill, total_oper,
    apenas_cbill, apenas_oper,
    pct_geral, pct_cbill_fora, pct_oper_fora,
):
    st.title("📊 Dashboard — Análise de Divergências")
    st.markdown(f"**📅 Data analisada: {data_filtro.strftime('%d/%m/%Y')}**")
    st.markdown("---")

    # ── métricas resumo ──
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Serviços Cbill",            total_cbill)
    m2.metric("Serviços Oper",             total_oper)
    m3.metric("Só no Cbill",               len(apenas_cbill))
    m4.metric("Só no Oper",                len(apenas_oper))

    st.markdown("---")
    st.subheader("🎯 Indicadores de Divergência")
    ...
    st.markdown("---")
    st.subheader("📊 Comparativo de Volumes")
...
aba_sidebar = st.sidebar.radio(
    "Navegação",
    options=["⚙️ Configurações", "📊 Dashboard"],
    label_visibility="collapsed",
)

with st.sidebar:
    if aba_sidebar == "⚙️ Configurações":
        ...
        st.header("📊 Dashboard")
        st.info("Os gráficos são exibidos na tela principal após o processamento das bases.")
        data_filtro = st.date_input("Data limite", value=datetime.today(), key="data_dash")

# ─── estado compartilhado ────────────────────────────────────────────────────
# Guarda resultados no session_state para o dashboard acessar
if "resultado_pronto" not in st.session_state:
    st.session_state["resultado_pronto"] = False

# ─── tela principal ───────────────────────────────────────────────────────────

if aba_sidebar == "📊 Dashboard":
    if st.session_state.get("resultado_pronto"):
        exibir_dashboard(
            data_filtro         = st.session_state["data_filtro"],
            total_cbill         = st.session_state["total_cbill"],
            total_oper          = st.session_state["total_oper"],
            apenas_cbill        = st.session_state["apenas_cbill"],
            apenas_oper         = st.session_state["apenas_oper"],
            pct_geral           = st.session_state["pct_geral"],
            pct_cbill_fora      = st.session_state["pct_cbill_fora"],
            pct_oper_fora       = st.session_state["pct_oper_fora"],
        )
    else:
        st.title("📊 Dashboard")
        st.warning("⚠️ Nenhuma análise processada ainda. Vá em **⚙️ Configurações**, faça upload das bases e processe para ver os gráficos aqui.")

else:
    # ─── aba configurações: fluxo normal ─────────────────────────────────────
    ...
    if arquivo_cbill and arquivo_oper_com:
        try:
            ...
            # ── salva no session_state para o dashboard ──
            st.session_state.update({
                "resultado_pronto": True,
                "data_filtro":      data_filtro,
                "total_cbill":      total_cbill,
                "total_oper":       total_oper,
                "apenas_cbill":     apenas_cbill,
                "apenas_oper":      apenas_oper,
                "pct_geral":        pct_geral,
                "pct_cbill_fora":   pct_cbill_fora,
                "pct_oper_fora":    pct_oper_fora,
            })
